...
class SpecialDownloadDeck extends \UnlistedSpecialPage {
	 ...
	 * Show Special Page
	 *
	 * @param string $subpage name of sub page
	 * @return void
	 */
	public function execute( $subpage ) {
		$pageid = $this->getRequest()->getInt( 'pageid' );
		// defaults to 0 unless deck is passed
		$num = $this->getRequest()->getInt( 'deck' );
	 ...
	 *
	 * @param string $deck deck number
	 * @param string $name deck name
	 * @param bool $setinfo toggle set info
	 * @return void
	 */
	public static function handleDeckApprentice( $deck, $name, $setinfo = false ) {
	 ...
	 * @return string
	 */
	public static function handleDeckMagicOnline( $deck ) {
		$deck = trim( $deck );
		$sections = preg_split( "/\n{2,}/", $deck );
		foreach ( $sections as $section ) {
			if ( preg_match( "/side|board|sb/i", $section ) ) {
	 ...
	 * @return sting
	 */
	public static function handleDeckOCTGN2( $deck, $name, $author ) {
